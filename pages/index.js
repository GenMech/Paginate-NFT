...
const endPoint =
  "https://dimensional-alpha-voice.solana-mainnet.discover.quiknode.pro/b3ece9aee0fdbd1d0bae1d5ba6841dc8c923240d/";
...
const Home = () => {
  ...
  useEffect(() => {
    ...
    const execute = async () => {
      ...
      setCurrentView(nftList.slice(startIndex, endIndex)); // The slice() method returns selected elements in an array, as a new array.
      setLoading(false);
    };
    execute();
  }, [nftList, currentPage]);
  ...
  const loadData = async (startIndex, endIndex) => {
    const nftsToLoad = nftList.filter((nft, index) => {
      // The filter() method creates a new array contains elements that pass a test provided by a function.
      return (
        index >= startIndex && index < endIndex && nft.metadataTask.isPending() // To check kiska metadeta load hona bacha hai index range me
      );
    });
